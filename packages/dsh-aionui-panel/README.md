...
- **File-tree context menu**: right-clicking a file or folder opens a menu — copy path, copy name, **copy / cut / paste** (panel clipboard via backend `/aionui-panel/copy` and `/aionui-panel/move` routes), reveal in file manager, open with default app (files only), **choose app to open with…** (system "Open With" dialog, `rundll32 OpenAs_RunDLL` on Windows), rename, new file, new folder, delete (with confirmation); every operation goes through the workspace gate (loopback fence + .git refusal), and reveal uses `explorer /select` on Windows, `open -R` on macOS, falling back to opening the parent directory on Linux desktops.
- **Single-click OS open**: a single click on a Word/Excel/PPT/PDF/image file (types with no usable in-app preview) hands it straight to the system **"Open With"** dialog instead of the unsupported preview panel. Paths containing **spaces** fall back to PowerShell `Start-Process` (opens with the default app), since `OpenAs_RunDLL` silently fails on paths with spaces — a long-standing Windows shell limitation.
- **Launch failure detection**: the host verifies the OS command actually launched by observing a 600ms grace window. WSL interop disabled (binfmt_misc entry missing) causes the process to exit early; the UI shows a toast notification ("Could not launch the external app") instead of a silent dead click.
